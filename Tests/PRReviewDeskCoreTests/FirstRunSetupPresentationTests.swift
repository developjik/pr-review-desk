...
enum FirstRunSetupPresentationTests {
    static func run() throws {
        ...
        try testCodexSignInIsFoldedIntoAIReviewSetup()
    }
    ...
    private static func testCodexSignInIsFoldedIntoAIReviewSetup() throws {
        let steps = FirstRunSetupPresentation.steps(
            hasGitHubCredential: true,
            isGitHubReady: true,
            isCodexCLIReady: true,
            isCodexChatGPTLoginReady: false,
            isPrivacyAcknowledged: true
        )
        ...
        try expectEqual(steps[1].state, .needsAction)
        try expectEqual(steps[1].detail, "Codex is installed. Finish Codex sign-in before generating reviews.")
        try expectEqual(steps[1].actionTitle, "Check Codex")
    }
}
